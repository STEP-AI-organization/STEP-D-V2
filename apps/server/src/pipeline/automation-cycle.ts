      ...
      for (const clip of mine) {
        ...
        const outcome = await dispatchPublish({
          ...
          ...(isNaver ? {
            ...
            // ⚠️ **카테고리는 여기서 정하지 않는다.** 예전엔 `엔터/엔터` 를 박아 넘겨서
            // 프로그램에 카테고리를 정해 둬도 자동배포는 늘 무시했다(페이로드가 이기니까).
            // 안 넘기면 워커가 프로그램 기본값 → 장르 유도 순으로 푼다 — 해석 경로가 하나다.
          } : {}),
...
): Promise<{ code: RuleIdleCode; detail: string } | null> {
