...
export interface MetaSource {
  ...
  titlePrompt?: string;
  /**
   * 쇼츠(9:16)인가 — 쇼츠면 해시태그·태그를 **넓게(핵심 + 인접 확장)** 뽑는다.
   *
   * 유튜브 쇼츠는 메타데이터·스크립트 임베딩이 추천의 1차 신호라, 같은 주제군의 인접 태그를
   * 넓게 달면 관련 영상 추천 풀에 함께 태워진다(사용자 2026-08-21 · 레퍼런스: 경제 쇼츠의
   * 20여 개 해시태그 블록). 16:9 클립은 좁게(5~8개) 유지 — 롱폼 추천은 다른 축이다.
...
export function buildMetadataPrompt(src: MetaSource): string {
  ...
  // 쇼츠는 설명 프로세를 길게 적지 않는다(사용자 2026-08-21 · 레퍼런스: 제목+해시태그 위주).
  // 유튜브가 스크립트를 따로 임베딩하므로 설명은 핵심 한두 줄이면 되고, 유입은 해시태그가 진다.
  const descLineGuide = isShort
    ? "- description: **짧게 — 한두 문장이면 충분하다.** 쇼츠 설명은 길게 늘여 쓰지 않는다(핵심만). 해시태그는 여기 넣지 말고 hashtags 로 분리한다."
    : "- description: 3~5문장. 해시태그는 여기 넣지 말고 hashtags 로 분리한다.";
  const tagsGuide = isShort
    ? "- tags: **10~15개.** 인물명·프로그램명·핵심 소재 + 같은 주제군의 확장 키워드까지. 앞에 # 을 붙이지 않는다."
    : "- tags: 5~10개. 인물명·프로그램명·상황 키워드. 앞에 # 을 붙이지 않는다.";
  const hashtagGuide = isShort
    ? "- hashtags: **15~25개.** 쇼츠의 추천 유입축이라 넓게 뽑는다. 두 겹으로 구성한다:\n" +
      "    · **핵심 태그(먼저):** 이 클립의 프로그램·인물·핵심 소재·장면 상황 — 위에 주어진 사실에서 나온 것.\n" +
      "    · **확장 태그(뒤):** 같은 **주제군**의 인접·상위 키워드 — 같은 분야의 관련 인물·작품·기관·사건·개념. " +
      "클립에 직접 안 나왔어도 같은 토픽이면 단다(추천 알고리즘이 관련 영상 풀에 함께 태우게).\n" +
      "    ⚠️ 해시태그는 **사실 서술이 아니라 검색·추천 분류 신호**다 — 위 [절대 규칙]의 사실 날조 금지는 " +
      "제목·설명 **본문**에 적용된다. 다만 클립과 **전혀 무관한** 주제(무작정 인기 키워드)는 달지 마라. 같은 주제군 안에서만 넓힌다.\n" +
      "    · 모두 # 을 붙이고 태그 안에 공백을 넣지 않는다(`#SK그룹` O · `#SK 그룹` X)."
    : "- hashtags: 5~8개(추천 해시태그). # 을 붙인 형태. 프로그램·인물·상황·감정 키워드를 폭넓게. 태그 안에 공백 금지.";

  return joinBlocks([
    // ① 역할 — 2026-09-16 사용자: "너무 진지한 내용분석체로 나온다" → 유튜브체로. 사실 규칙(⑧)은
    //    그대로 두고 **문체 축만** 바꾼다 — 날조 금지와 톤은 별개다.
    "너는 한국 방송 클립을 유튜브·SNS 에 올리는 채널 운영자다. 사실만 쓰되 문체는 **유튜브 감성**이다 — " +
    "내용을 분석·정리한 보고서가 아니라, 피드를 넘기던 엄지를 멈추게 하는 문구를 쓴다.",
    ...
    // ⑧.2 톤 — 유튜브체 + 이모지 (사용자 2026-09-16). 조건 없이 항상 들어간다 — 쇼츠·클립·
    //    다국어 전부 같은 톤이고, wantProductQueries 양쪽에 똑같이 실리므로 commerce.test.ts 의
    //    "게이트 OFF 는 프롬프트 불변" 검사와도 충돌하지 않는다.
    "[톤 — 유튜브체]\n" +
    "- 내용 분석·보고서 문체 금지. 시청자에게 말 걸듯 쓴다(\"~하는 순간\", \"~해버린 이유\", \"이게 되네?\").\n" +
    "- **이모지를 쓴다**: 제목에 1~2개, 설명에 1~3개 — 장면의 감정에 맞는 것만(😂🔥😱🥹💕 등). " +
    "같은 자리 연속 3개 이상은 스팸으로 보인다.\n" +
    "- ?!·…·짧은 감탄은 자유. 궁금하게 만들되, [절대 규칙]의 사실 날조 금지는 그대로다.",
...
export function clip(text: string, max: number): string {
  ...
  // 이모지는 서로게이트 페어(2유닛)라 slice 가 한가운데를 자를 수 있다 — 반쪽(고위 서로게이트)이
  // 남으면 깨진 문자가 제목에 그대로 나간다. 톤 규칙이 이모지를 쓰게 하므로(2026-09-16) 실제로
  // 밟히는 경로다. 반쪽이면 한 칸 물린다.
