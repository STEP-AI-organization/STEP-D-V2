...
export type JobType =
  ...
  | "clip.render"
  /**
   * 원본을 **프리미어가 읽을 수 있는 코덱(H.264)** 으로 바꾼다.
   *
   * 왜 필요한가 (실측 2026-08-31): 유튜브에서 받은 원본이 **VP9-in-MP4** 였다. ffmpeg 도
   * 우리 파이프라인도 멀쩡히 돌지만 **프리미어는 MP4 안의 VP9 를 못 읽는다** — 편집자
   * 화면에는 오디오 파형만 뜬다. 편집자에게 원본을 넘기는 경로(프리미어 패널)가 생긴
   * 이상 이건 우리가 고칠 문제다. 새 다운로드는 avc1 을 먼저 고르게 고쳤고(worker.ts),
   * **이미 받아 둔 것**은 이 잡이 훑어 바꾼다.
   *
   * 렌더 레인에 둔다 — CPU 를 통째로 쓰는 성격이 clip.render 와 같고, 노는 사무실 PC 가
   * 그 레인을 당겨간다. 클라우드 워커의 시간을 여기 쓸 이유가 없다.
   */
  | "media.transcode"
  ...
  | "gebd.detect"
  // Registered-cast YOLO/ArcFace inference on the L4 GPU lane.
  // The CPU content worker only produces beats and queues this follow-up.
  | "cast.detect";
