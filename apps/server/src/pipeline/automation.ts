...
export interface AutomationRule {
  ...
  layout?: {
     ...
     *
     * 분석은 이 값을 보고 알아서 그 언어 자막을 만든다(content-pipeline.resolveTranslateLangs)
     * — 따로 켤 스위치가 없다. 다만 **이미 분석이 끝난 회차**는 번역이 없으므로 한국어 자막이
     * 그대로 나간다(조용히 — 발행을 막지는 않는다). 그 회차는 재분석해야 언어가 붙는다.
     */
    lang?: string;
  };
...
/**
 * 사무실 PC 에 **한 번에 걸어 둘 렌더 수**. 이만큼 안 끝난 게 쌓이면 나머지는 클라우드가
 * 직접 굽는다 — 즉 **둘이 나눠서 한다**(사용자 2026-09-02 "윈도우2랑 클라우드랑 나눠서").
 *
 * 예전엔 `RENDER_VIA_QUEUE=1` 이면 **전부** 큐로 갔다. 클라우드는 큐가 10분 넘게 정체될 때만
 * 끼어들었으니, 그건 분담이 아니라 **장애 대비**다. 사무실 PC 는 렌더를 직렬로 굽는데
 * (건당 50~90초) 순방은 한 틱에 최대 8건(AUTOMATION_MAX_RENDERS_PER_TICK)을 만들므로,
 * 전부 넘기면 뒤쪽 건은 PC 앞에 줄만 서고 클라우드 CPU 는 논다.
 *
 * 깊이로 가르는 이유: 비율(50:50 같은 고정값)은 PC 가 느려지거나 꺼져도 계속 같은 몫을
 * 보내지만, 깊이는 **PC 가 실제로 소화하는 만큼만** 준다. PC 가 꺼지면 깊이가 즉시 차서
 * 나머지가 전부 클라우드로 간다 — 10분 정체를 기다릴 필요도 없어진다.
 *
 * 기본 3 — PC 가 3건(≈3.5분) 앞서 들고 있게 하되 그 이상은 안 쌓는다. 0 으로 두면 큐를
 * 아예 안 쓴다(= 전부 클라우드). 오타·음수는 기본값으로 떨어진다.
 *
 * ⚠️ 사무실 PC 가 공짜인 건 CPU 뿐이다. 원본을 GCS 에서 받아 오므로 인터넷 egress(≈₩165/GB)가
 *    붙는다 — 같은 리전 Cloud Run 은 그게 0원이다(CLAUDE.md). 이 값을 올릴수록 CPU 는
 *    아끼지만 전송비는 늘어난다.
 */
export function renderQueueMaxPending(): number {
  // ⚠️ 빈 문자열을 먼저 걷어낸다. `Number("")` 은 0 이라, env 를 빈 값으로 두면 "0 = 큐를
  //    아예 안 쓴다" 로 읽혀 **사무실 PC 가 조용히 꺼진다.** 0 은 사람이 명시했을 때만이다.
  const raw = String(process.env.RENDER_QUEUE_MAX_PENDING ?? "").trim();
  if (!raw) return 3;
  const n = Number(raw);
 ...
 *
 * ⚠️ **한 건도 못 나간 슬롯은 조용히 사라진다** — 그 슬롯이 목표에서도 빠지므로 리포트
 * 메일에 "아침 2건 못 나감" 이 적히지 않고, 그날 아무것도 안 나갔으면 메일 자체가 없다.
 * 사용자 확정 2026-09-02: **알릴 필요 없다.** 알리게 만들려면 목표를 "창이 열렸던 슬롯의
 * 계획 수" 로 바꿔야 하는데, 그러면 다음 슬롯 창이 리드(2시간)로 미리 열리면서 목표가
 * 앞당겨 올라가 **슬롯별 메일이 다시 하루 단위로 뭉친다**(아침 메일이 저녁까지 대기).
 * 둘은 동시에 가질 수 없고, 슬롯별 메일 쪽을 골랐다. 순방 유휴 사유(rule_run)에는 남는다.
 *
