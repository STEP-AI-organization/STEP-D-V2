...
export function autoTopupActionHint(code: AutoTopupCode): string {
  switch (code) {
    ...
    case "bad_amount":
      return "자동 충전 금액을 만들 수 없습니다 — 자동 충전 설정에서 충전량을 다시 지정하세요.";
    ...
    case "price_unset":
      return "결제 단가가 설정되지 않아 자동 충전을 할 수 없습니다 — 담당자에게 문의해 주세요."
        + " 자동 충전 설정을 바꿔도 풀리지 않습니다.";
    case "bad_policy":
      return "충전량이 임계 이하라 충전해도 임계를 못 넘습니다 — 충전량을 임계보다 크게 바꾸세요.";
    case "daily_cap":
      return "오늘 자동 충전 횟수 상한에 도달했습니다 — 지금 필요하면 직접 충전하고, 상한은 자동 충전 설정에서 조정하세요.";
    case "monthly_cap":
      return "이번 달 자동 충전 금액 상한에 도달했습니다 — 직접 충전하거나 월 상한을 조정하세요.";
    default:
      return "";
  }
}
