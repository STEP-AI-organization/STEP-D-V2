...
export const API_KEY_ROUTES: RouteRule[] = [
  ...
  { method: "GET", path: /^\/api\/shorts-templates\/[^/]+\/overlay\.png$/, scope: "factory:read" },
  ...
  // 미리보기 기하 — **저장하지 않는다**(읽기 전용). POST 인 건 초안(lines·layout)을 본문에
  // 실어야 해서지 쓰기라서가 아니다. 콘솔이 제목 배치를 베껴 두면 렌더가 바뀔 때 조용히
  // 갈라지므로, 렌더가 쓰는 함수를 그대로 태워 숫자만 내려준다(사용자 2026-09-15:
  // "미리보기랑 실제 렌더된 영상 괴리가 있으면 안 된다").
  { method: "POST", path: /^\/api\/clips\/[^/]+\/overlay-preview$/, scope: "factory:read" },
