...
async function proxy(request: NextRequest, paramsPromise: Promise<{ path?: string[] }>) {
  ...
  try {
    ...
    // ⚠️ **keep-alive 소켓 재사용이 "fetch failed" 의 뿌리다.** Vercel 서버리스 함수가 freeze/thaw
    // 되면 undici 풀에 남은 소켓이 죽어 있어, 그걸 재사용한 요청이 ECONNRESET 으로 죽는다(저장·
    // 렌더·채택이 "됐다 안 됐다" 하던 그것). 예전엔 그걸 **재시도**로 흡수하려 했는데, 재시도 2차에서
    // undici 가 **UND_ERR_INVALID_ARG** 로 던지는 새 병이 생겼다(warm 함수의 재시도 경로에서만 ·
    // 외부 curl 로는 재현 불가 · 사용자 2026-08-21 "채널규칙·저장 fetch failed (UND_ERR_INVALID_ARG)").
    // → 재시도를 없애고 **근본을 친다: Connection: close 로 소켓을 안 남긴다.** 매 요청이 새 연결이라
    //   재사용할 죽은 소켓이 없다 = 두 오류(ECONNRESET·UND_ERR_INVALID_ARG) 모두 원천 차단.
