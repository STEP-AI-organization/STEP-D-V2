/**
 * ============================================================================
 * VENDORED FROM opencut-app/opencut-classic (MIT License, see ./LICENSE)
 * ============================================================================
 * Source: apps/web/src/timeline/ruler-utils.ts (archived 2026-05-17)
 * Adapted for STEP-D by removing opencut-internal dependencies:
 *   - opencut-wasm::FrameRate  → local type (num/den)
 *   - @/timeline/scale         → local constant BASE_TIMELINE_PIXELS_PER_SECOND
 *   - @/fps/utils              → local frameRateToFloat helper
 *
 * The pattern (2·3·5·10·15 frame intervals) matches CapCut per the original comment.
 * Copyright (c) OpenCut contributors. MIT licensed.
 */

/** Frame rate as numerator/denominator (e.g. 30/1, 30000/1001 for NTSC). */
export type FrameRate = { readonly numerator: number; readonly denominator: number };

/** Timeline base scale. STEP-D uses 60px/second at zoom=1 (tuned to our footer height). */
const BASE_TIMELINE_PIXELS_PER_SECOND = 60;

function frameRateToFloat(fps: FrameRate): number {
  const d = fps.denominator || 1;
  return fps.numerator / d;
}
