"""Exp 9 ENA — v2 결정론 필터 (LLM 판정 X). Exp 8 v2와 동일 5신호 AND.

입력:
- picks: 5 longs × 6 shorts = 30 pick 후보 (D:/STEPD-experiments/ena_longs_analysis/*/analysis.json)
- truth: 로컬 매칭 7 pairs (exp9_ena_truth.json)
- retention: 5 longs 유지율 커브 (exp9_ena_retention.json)

5신호 (Exp 8 v2와 동일):
  1) IoU ≤ 0.1 (편집자 미발견)
  2) rel_vs_whole ≥ 1.05
  3) 길이 30~60초
  4) 텍스트 밀도 ≥ 3.0 chars/s
  5) hook ∈ {돌직구, 반전, 감정고조, 갈등, 웃음, 공감}
"""
import csv
import json
import sys
...
DATA_DIR = Path("D:/STEPD-experiments")
OUT_DIR = DATA_DIR / "results"
REPORT_DIR = Path(r"C:\Users\STEPAI05\STEPD-repo\바우처_결과보고_2026\증빙_데이터셋\실험자료")

# 로컬 매칭 truth
truth_bundle = json.load(open(OUT_DIR / "exp9_ena_truth.json", encoding="utf-8"))
...
# 리텐션
RET = json.load(open(OUT_DIR / "exp9_ena_retention.json", encoding="utf-8"))
...
# 5 longs의 analysis + stt 로드
longs_dir = DATA_DIR / "ena_longs_analysis"
long_data = {}
for lid_dir in longs_dir.iterdir():
...
for lid, d in long_data.items():
    analysis = d["analysis"]
    stt = d["stt"]
    long_dur = float(analysis.get("duration", 0)) or float(RET.get(lid, {}).get("dur", 0))
...
# CSV
CSV_OUT = OUT_DIR / "exp9_ena_deterministic_picks.csv"
if all_records:
...
JSON_OUT = OUT_DIR / "exp9_ena_confirmed_gems.json"
...
print("\n=== 하하 v2와 대조 ===")
print(f"           하하  ENA")
print(f"전체 픽:     46    {len(all_records)}")
print(f"통과:       8     {len(winners)}")
print(f"통과율:    17%    {int(100*len(winners)/max(1,len(all_records)))}%")
