...
> **어느 계층에 넣을지**는 [CLAUDE.md](CLAUDE.md) "컨벤션 — 검증을 어디에 넣는가" 가 정본이다.
> 계층이 넷(순수 함수 · 소스 스캔 · e2e · 실브라우저 스모크)이고, 각 계층이 **증명 못 하는
> 것**이 표로 정리돼 있다. 여기서는 돌리는 방법만 적는다.
>
> 한 줄 요약: **"깨졌을 때 어떻게 드러나는가"** 로 고른다. 값이 틀리면 순수 함수,
> 아무 일도 안 일어나면(조용한 실패) 소스 스캔 **+** e2e 둘 다, 남의 데이터가 보이면 e2e,
> 브라우저에서만 깨지면 스모크.

순수 함수는 평범하게 쓰면 된다. 다만 **이 리포 테스트의 상당수는 소스를 읽는다** —
`publish-guard` · `worker-lanes` · `docs-drift` · `rls-access` · `caption-font` 같은 것들
(실측 2026-09-15: 108파일 중 44가 소스 스캔, 그중 19는 웹 소스까지 읽는 경계 검사).
...
### 스모크 — 진짜 크롬 · 진짜 프로덕션 빌드 (2026-09-14 신설)

`pnpm check` 에는 **안 들어간다.** 빌드가 필요해서 CI 의 `web-build` job 에서 돈다.

```bash
pnpm --filter @stepd/web exec playwright install chromium   # 처음 한 번
pnpm --filter @stepd/web smoke
```

`next build`(프로덕션 설정) → `next start` → 크롬으로 두드림 → 정리까지
`apps/web/scripts/smoke.mts` 하나가 다 한다. API 는 Playwright 라우트 가로채기로 흉내낸다 —
묻는 것은 "서버가 옳게 답하는가"(그건 e2e 몫)가 아니라 **"브라우저가 옳게 부르는가"** 다.

**왜 필요했나:** 웹 관문이 오래 `next build` 뿐이었다. 그건 타입과 프리렌더만 본다. 실제로
워커의 `page.evaluate` 에 esbuild `__name` 이 딸려가 브라우저에서 전량 실패한 사고가 있었고
**유닛으로는 안 잡혔다**.

⚠️ **화면의 글자·DOM 은 일부러 안 본다.** 프론트 개편 중이라 셀렉터에 걸면 매주 빨개지고,
그러면 사람이 관문을 무시하게 된다. 보는 것은 미들웨어 리다이렉트 · 어느 엔드포인트를
부르는가 · 304 를 받아도 안 무너지는가.

⚠️ 스모크 파일은 `*.smoke.ts` 다. `*.test.ts` 로 만들면 `pnpm test` 가 집어가서 **브라우저도
서버도 없이 돌다 실패한다.**
...
| 무엇 | 규칙 |
|---|---|
| 서버 라우트 | 도메인별 **`src/<도메인>/routes.ts`** 로 옮기는 중 (2026-09-14 방침 전환 — 그전엔 "index.ts 한 파일"). 절차와 함정은 [CLAUDE.md](CLAUDE.md) "컨벤션 — 라우트를 도메인 폴더로 옮기는 절차" 가 정본 |
...
> 나머지 스타일 관용구 일곱 개(실패 방향 · 사고 이력 주석 · 사유를 삼키지 않기 · 응답을
> 필드 단위로 조립 · 바이트가 돈 · env 최소화 · 숫자엔 날짜)는 [CLAUDE.md](CLAUDE.md)
> "컨벤션 — 코드 스타일" 에 근거와 함께 모아 뒀다. **새로 정한 규칙이 아니라 이미 코드에
> 있는 것을 귀납한 것**이라, 읽으면 "왜 여기선 이렇게 쓰나" 가 대부분 풀린다.
