...
**원가(2026-08-19 갱신)**: 프로덕션 현재 구성 60분 회차 ≈ **₩800(분당 ₩13.3)** → 판매가 ₩28/분
대비 **마진 약 51%**. 2026-08-19 beat_annot(장면이해 본체)를 **gemini-2.5-flash-lite** 로 전환해
...
(env `=0`) — 화자 실명·인물 검색축이 없다. 켜도 flash-lite 면 흑자(마진 ~21% · +₩440).
...
| `src/worker.ts` | **워커 프로세스 진입점.** 잡 21종 · 레인 4개 · drain 모드 (아래 참조) |
