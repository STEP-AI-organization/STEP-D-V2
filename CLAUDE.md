...
**원가(2026-08-17 실측 확정)**: 프로덕션 현재 구성 60분 회차 ≈ **₩1,124(분당 ₩18.7)** →
판매가 ₩28/분 대비 **마진 31%**. 갈림길은 화면 자막 읽기(`RUN_CHYRON_PER_SEG`)다 —
**프로덕션 잡 env 에 `=0` 이 박혀 꺼진 채로 돌고 있다**(즉 화자 실명·인물 검색축이 없다).
켜면 분당 ₩39.8(편당 ₩759 적자), 켜고 배치 모드(`GEMINI_BATCH=1`)까지면 분당 ₩28.9.
