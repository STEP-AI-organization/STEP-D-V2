...
**원가(2026-08-17 실측 확정)**: 60분 회차 ≈ **₩2,385(분당 ₩39.8)** — 판매가 ₩28/분보다
크다, 즉 **현재 프로덕션 기본값으로는 편당 ₩759 적자다.** 갈림길은 화면 자막 읽기
(`RUN_CHYRON_PER_SEG` · 프로덕션 기본 ON · 원가의 51%)를 켜느냐다 — 끄면 분당 ₩18.7 · 마진 31%.
⚠️ 리포 곳곳에 돌던 ₩285·₩994·₩1,510 은 **전부 틀린 값**이다. 세 번 다 같은 방향으로
틀렸다 — **로그에 안 찍힌 스테이지를 0 으로 셌다.** 원가를 인용하기 전에 그 로그가 전체
실행인지(체크포인트 재개 아닌지), 꺼진 스테이지가 없는지 볼 것.
...
| `src/worker.ts` | **워커 프로세스 진입점.** 잡 19종 · 레인 4개 · drain 모드 (아래 참조) |
