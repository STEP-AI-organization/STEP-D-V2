...
> 2026-09-15 고정. 이 리포엔 검증 계층이 **넷**이고, 잘못 고르면 "초록인데 안 돈다" 가 된다.
> 실측(2026-09-15 · `apps/server/src/tests/`): 총 108파일 중 **소스 스캔 44** · 그중
> **웹 소스까지 읽는 경계 검사 19** · 문서 스캔 2 · 나머지는 순수 함수. e2e 는 1파일(19개).

| 계층 | 증명할 수 있는 것 | **증명 못 하는 것** |
|------|------------------|---------------------|
| 순수 함수 (`*.test.ts`) | 계산·문구·조합·상태기계 | 배선. 함수가 **불리는지**는 모른다 |
| 소스 스캔 (`*.test.ts` + `readFileSync`) | "코드에 그 모양이 있다"·불변식·문서 일치 | **호출 여부**. 파일에 있어도 등록 안 됐을 수 있다 |
| e2e (`tests/e2e/*.e2e.ts`) | 진짜 DB·서버에 HTTP — RLS 격리, 라우트 등록, 응답 모양 | 브라우저에서 벌어지는 일 |
| 스모크 (`apps/web/src/tests/smoke/*.smoke.ts`) | 실브라우저 — 미들웨어, 어느 엔드포인트를 부르는가 | 서버가 옳게 답하는가 |

**고르는 법 — 질문 하나로 갈린다: "이게 깨졌을 때 어떻게 드러나는가?"**

- 값이 틀린다 → 순수 함수
- **아무 일도 안 일어난다**(조용한 실패) → 소스 스캔 + e2e **둘 다**
- 남의 데이터가 보인다 → e2e (RLS 는 DB 안에서만 증명된다. 안 세우면 에러가 아니라 **빈 결과**다)
- 브라우저에서만 깨진다 → 스모크 (실제 사례: 워커의 `page.evaluate` 에 esbuild `__name` 이
  딸려가 전량 실패했는데 유닛은 전부 초록이었다)

**⚠️ 소스 스캔의 두 함정** — 이 세션에서 실제로 밟았다:

1. **검사 대상이 사라지면 조용히 초록이 된다.** 라우트를 도메인 파일로 옮기자 `index.ts` 만
   읽던 검사가 0건을 스캔했다. `naver-credentials` 의 "비밀번호를 응답에 싣지 않는다" 같은
   **보안 검사**가 여기 걸렸다. 해법은 경로를 지우는 게 아니라 **범위를 넓히는 것** —
   `tests/sources.ts` 의 `routeSource()`·`sourceFiles()` 를 쓴다.
2. **라우트 블록 정규식은 `\n});`(열 0)로 끝을 잡는다.** 함수 안으로 들어가면 `\n  });` 라
   안 맞는다 → **`\n\s*\}\);`** 로 바꿀 것.

**스캐너가 죽었는지 확인하는 하한을 둘 것.** 정규식이 안 맞으면 "빈 목록 vs 빈 목록" 으로
**항상 통과**한다. `web-routes-exist` 의 "스캐너 자체가 살아 있다"(`size > 150`)가 그 예다.

## 컨벤션 — 라우트를 도메인 폴더로 옮기는 절차

분할 1호(챗봇·리포트 9개) · 2호(네이버 13개)에서 굳은 순서다. 그대로 따르면 된다.

```
1  앵커 찾기    줄번호 금지 — 다른 세션이 index.ts 를 동시에 고친다.
                `app.get("/api/<도메인>/…"` 같은 ASCII 경로로 찾고, 위쪽 `// ──` 구분선까지 올라간다.
2  파일 생성    src/<도메인>/routes.ts
                export function registerXRoutes(app: AppHono): void { …본문 2칸 들여쓰기… }
                헤더 주석(`// ──`)은 함수 밖에 둔다.
3  걷어내기     index.ts 에서 블록 제거 → 자리에 `registerXRoutes(app);` + "어디로 갔는지" 주석.
                import 추가. 흩어진 같은 도메인 라우트도 이때 같이 모은다.
4  심볼 해결    `npx tsc --noEmit` 이 없는 이름을 전부 알려준다. 그게 곧 import 목록이다.
                index.ts 로컬 헬퍼를 쓰면 → 순환이 되므로 공용 위치로 옮긴다
                (가드는 `auth/admin.ts`, 타입은 `app-env.ts`).
5  스캔 확장    깨진 테스트를 `routeSource()` 로 돌린다. 정규식 끝 앵커도 같이(위 함정 2).
6  등록 증명    api.e2e.ts 에 세션으로 200 받는 테스트 한 줄. **이것 없이는 완료가 아니다.**
7  검증         pnpm check → pnpm test:e2e → 라우트 수 세어 CLAUDE.md·api-reference.md 갱신.
```

**왜 `app.route()` 서브앱이 아닌가** — 서브앱은 경로가 마운트 기준 상대경로가 된다. 소스 스캔
59곳이 `app.post("/api/superadmin/tenants/:id/api-keys"` 같은 **전체 경로 문자열**을 grep 하므로
전부 무력화된다(`src/app-env.ts` 주석). 함수 등록이면 경로는 그대로고 들여쓰기만 바뀐다.

**아직 안 옮긴 것**(2026-09-15): `superadmin` 38 · `media` 33 · `programs` 21 · `clips` 21 ·
`youtube` 19 · `recommendations` 11 · `factory` 9 · `billing` 8 · `automation` 8 · `assets` 8 …
큰 덩어리는 `index.ts` 를 동시에 고치는 세션이 없을 때 잡을 것.

## 컨벤션 — 코드 스타일 (새로 정하지 않고 귀납한 것)

이 리포엔 이미 강한 관용구가 있다. 새 규칙을 얹기보다 **그걸 계속 쓰는 것**이 컨벤션이다.

1. **실패 방향을 안전한 쪽으로 잡는다.** 게이트는 오타·빈값·미설정이 전부 OFF(`upload-gate.ts`),
   테넌트 스코프가 없으면 조용히 새는 게 아니라 터진다(`db-pg.ts` 풀 프록시), 인증 자세가
   어긋나면 아예 서빙하지 않는다(`assertAuthPosture`). **새 스위치를 넣을 때 "오타났을 때
   무엇이 되는가" 를 먼저 정할 것.**
2. **주석에 사고 이력을 남긴다.** "2026-08-31 에 이래서 34.5GB 썼다" 가 규칙 옆에 있으면
   아무도 그 규칙을 함부로 지우지 않는다. 날짜·실측치·증상을 같이 적는다.
3. **사유를 삼키지 않는다.** 서버는 `{error, message}` 로 한국어 사유를 주고, 프론트는
   그걸 살려서 던진다(`api.ts` `json()`). `403 Forbidden` 만 보여주지 말 것.
4. **응답에 안 나갈 것은 필드 단위로 조립한다.** 엔티티를 통째로 펼치면 언젠가 값이 샌다
   (`naver/routes.ts` 헤더 — 세션·자격증명은 "있다/없다 + 시각" 만 나간다).
5. **바이트가 돈이다.** 프로덕션 웹은 `/api/proxy` 경유라 서버가 보내는 모든 바이트가 과금된다.
   상태 확인에 데이터를 받지 말고(`/health`), 자라는 응답엔 상한을 둔다(`STATE_JOB_LIMIT`).
6. **env 는 시크릿·인프라 위치만.** 제품 동작 스위치는 DB·요청 파라미터로. 새 env 를 넣기 전에
   "이게 배포 없이 바뀌어야 하는 값인가" 를 물을 것.
7. **숫자를 문서에 적을 땐 실측하고 날짜를 붙인다.** `docs-drift` 가 대조한다. 깨지면 숫자를
   지우는 게 아니라 다시 세서 갱신한다.
