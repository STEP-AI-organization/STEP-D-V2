...
|------|------|-----------|--------------|------|
| **A 수집·변환** | MXF/파일/URL 수용, 리먹스 분기, 분석용 프록시 | ffmpeg(리먹스 = ffprobe 코덱검사 H.264+AAC→`-c copy`), yt-dlp(URL), 프록시 480/720p + 16kHz mono wav | ffmpeg LGPL/GPL, yt-dlp Unlicense | 업로드(GCS 직접 resumable, 329844f)+서명 URL ffprobe. 프록시·리먹스·URL임포트 신규 |
...
| **C 훅 사전 후보** | 한국어 훅 사전 대조 + 로컬 점수(후보상한30·중첩0.35·프리롤2.5s) | **100% 자체 IP** + **Kiwi**(형태소, `kiwi-nlp` WASM) | Kiwi LGPL-3(SaaS 사용이라 무관) | 전부 신규(현 추천 = core AI 쇼츠 단일 경로, §3.2 — 훅 사전 로직은 아직 없음) |
...
- **E-1 (AP3 병행, 최소 편집기):** `opencut-classic`에서 **웨이브폼 + 타임코드 입력 + 드래그 스냅** 발췌 → 리트림 정밀화(종결어미 스냅 D와 궁합). **자막 오버레이 인라인 편집**(text-edit-overlay·transform-handles·snap-guides). `../prototypes/editor-prototype.html` 목업이 목표 UI. *(현황: 웨이브폼·타임코드·오버레이 직접조작은 커밋 201dd54에서 네이티브 재구현으로 이미 착지 — §3.4.)*
...
| 필요 | 시점 | 내용 |
|------|------|------|
| ~~GCS 서명 URL 업로드~~ | ~~AP1~~ **✅ 완료** | 커밋 329844f로 구현·실동작: `upload-init`(resumable 세션) → 브라우저→GCS 직접 업로드 → `finalize`(서명 URL ffprobe). 서버 메모리 경유 없음(§3.1) |
...
| ~~`yt-analytics.readonly` 스코프 추가~~ | **✅ 코드 완료** | `index.ts` `YT_ANALYTICS_SCOPES`에 `yt-analytics.readonly`+`yt-analytics-monetary.readonly` 포함(publish 스코프와 분리, `?mode=` 분기). 리텐션·수익 수집 실동작. 남은 것 = OAuth 동의화면 프로덕션 게시/구글 심사(코드 밖 절차) |
...
| 단계 | 콘텐츠(AP) | CX | 편집기 |
|------|-----------|----|--------|
| **AP1 골격** | ~~서명 URL 업로드~~(✅ 완료, 329844f), 프록시 생성, 리먹스 분기, 잡 체인(`media.ingest→analyze.stt→analyze.candidates→analyze.vision→clip.render→publish`) — **`clip.render`는 최종 확정 시 1회만 트리거(자동 아님, §2.4)**, GCS↔/tmp 수명관리 | — | E-0(채택 구간 재생·retrim 연결) |
| **AP2 진짜 추천** ★첫 체감가치·영업데모 | STT+`transcript`, 훅 사전 C, 경계 스냅 D, ~~(A)/(B) 조인~~(✅ 완료, 5197193 — AI 쇼츠가 추천 보드에 직결·휴리스틱 폐지, §3.2). **프로그램·채널 프로파일 스키마 + 하드 제약 필터 + 클립 인물 태그 자리 선반영**. **[이 단계 렌더 없음 — 전부 메타]** | CX1 착수 준비 | — |
...
채택은 AI 추천 구간·결정을 확정하는 **메타 연산**으로 바꾸고(현재는 채택 즉시 ffmpeg 트림·인코딩 = 불변식 위반, index.ts `/adopt`), 실제 트림·인코딩은 **최종 확정/익스포트 시 1회**로 이연(render revision 해시 캐시). 프리뷰는 원본 스트림 Range + CSS 오버레이 근사로.
