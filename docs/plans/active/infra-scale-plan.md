...
|---|---|---|---|---|---|
| ~~① GPU VM 2호기~~ | **2026-09-16 적용됨**(사용자 지시 — DB 스펙업 원복분을 여기로) — 1호기 머신 이미지 복제로 `stepd-gebd-vm-2` 생성(us-central1-**a** · -b 는 L4 spot 품절). gebd·cast 레인 자동 복제 · wake 라우트가 `이름:존` 다중 VM 지원 | — | ₩13,800+가동 반영 | — |
...
- **2026-09-16 같은 날 갱신**: ③ DB 스펙업 선제 적용 → **7일 실측(CPU p95 21%)을 보고 같은 날 원복**,
  그 예산으로 ① GPU 2호기를 대신 적용(사용자 결정 — "가격 대비"). §2 감시 배선 중 Monitoring
  알림 2개(SQL CPU·worker-kick 실패 → hkj@stepai.kr 메일) 생성됨. 남은 §2 항목: 큐 대기 p95 주간 점검.
