...
>
> ⚠️ 아래 §요약·§인터페이스의 **WebView + 메시지 브리지 항목은 2026-08-28 폐기됐다**
> (사용자 확정: 네이티브 패널). 원문은 접수 기록으로 남겨 두므로, 착수 전에 §부록의
> "첫 슬라이스" 를 먼저 읽을 것.
...
3. 회차·추천 목록도 **패널에서 직접** 그린다 — 기존 REST(`/api/state`·`/api/media/:id/analysis`)를
   그대로 부른다. (구 계획의 `stepd.stepai.kr/premiere` 웹 화면은 만들지 않는다 — 아래 결정)
4. 프리미어 조작 배선 — 프로젝트 조회 → 미디어 연결 → 재생위치 이동 · 추천 마커 생성
5. 내보내기 자동화 — 활성 시퀀스를 AME 프리셋으로 렌더 → 지금의 업로드 경로에 물리기
...
**추진안에서 벗어난 점 하나 — WebView 가 아니라 UXP 네이티브 패널이다.**
...
> **2026-08-28 사용자 확정: 네이티브 패널로 간다.** 처음엔 "화면이 커지면 WebView 로
> 갈아탄다" 고 유예해 뒀지만, 그 유예를 접는다. 그래서 **`stepd.stepai.kr/premiere` 웹 화면과
> 메시지 브리지(`PROJECT_CONTEXT`/`LINK_MEDIA`/… discriminated union)는 만들지 않는다** —
> 추천 목록도 패널이 REST 를 직접 부른다. 얻는 것: 웹 배포와 플러그인 릴리스가 안 엮이고
> (Vercel 이 죽어도 패널은 뜬다), 세션을 두 런타임 사이로 넘기는 표면이 아예 없다.
> 대가: 화면을 웹 컴포넌트로 재사용하지 못하고 UXP 가 지원하는 HTML/CSS 부분집합 안에서
> 그려야 한다. 지금 화면 규모에선 대가가 더 싸다.
