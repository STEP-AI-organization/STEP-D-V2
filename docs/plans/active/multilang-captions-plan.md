...
**켤 곳이 하나다** — 자동배포 계획의 `layout.lang`.

```
자동배포 계획  layout: { "lang": "vi" }
     ↓
분석이 그 계획을 보고 알아서 베트남어 자막을 같이 만든다
     (content-pipeline.resolveTranslateLangs → core --translate-langs)
     ↓
그 계획이 만드는 클립의 자막·제목·메타·글꼴·캡션트랙이 전부 베트남어
```

**env 스위치를 두지 않았다.** 켜는 곳이 둘이면 반드시 한쪽만 켜지고 그 실패가 조용하다 —
"번역만 쌓이고 안 쓰임" 또는 "계획은 베트남어인데 한국어가 나감". 계획이 곧 스위치면
그 어긋남이 성립하지 않는다.

⚠️ **이미 분석이 끝난 회차**는 번역이 없다 — 계획에 언어를 넣어도 그 회차는 한국어 자막이
그대로 나간다(발행을 막지는 않는다). 재분석해야 언어가 붙는다.
...
### 2026-09-14 추가 — 인도네시아어·영어 + 화면 픽커

**목표는 K콘텐츠의 해외 채널 배포이고, 1순위 언어는 인도네시아어다**(사용자 확정).

폰트 cmap 을 다시 실측했다(§9 방법 · 번들 11종 · 후보 11개 언어):

| 언어 | 번들 커버리지 | 판정 |
|---|---|---|
| 인도네시아어·영어·스페인어·포르투갈어 | **11종 전부 100%** | ✅ 폰트작업 0 · **글꼴 제한도 불필요** |
| 베트남어 | 2종만 100% | ✅ (기존 · `allowFonts` 로 제한 중) |
| 일본어 | 가나 98~100% / **한자 0%** | ❌ 폰트 번들 선행 |
| 중국어·태국어·힌디어·아랍어 | **0~1%** | ❌ 폰트 번들 선행 |

인도네시아어가 베트남어보다도 싸다 — 성조가 없어 **검은고딕·주아·도현까지** 덮는다.
그래서 `allowFonts: []`(제한 없음)이고 썸네일 대체표에도 항목이 없다.

글자 폭도 같은 방법으로 쟀다(기존 값 0.864·0.585·0.552 를 **재현해 방법 일치를 먼저 확인**):

| 언어 | widthEm | captionMaxChars | titleMaxChars |
|---|---|---|---|
| 인도네시아어 | 0.535 | 18 | 65 |
| 영어 | 0.524 | 18 | 66 |

**UI 가 생겼다.** 자동배포 계획 화면에 언어 픽커(`PUBLISH_LANGS_UI`)를 넣었다. 비한국어를
고르면 "이미 분석이 끝난 회차는 재분석해야 한다"는 경고가 같이 뜬다 — 이 기능의 가장 흔한
오해라서다. 서버 표와 갈라지지 않게 `caption-lang.test.ts` 가 코드 집합 일치를 강제한다.

### 아직 안 한 것

- **회차 전체 번역 스모크 미실시.** 10줄만 실호출했다 — 925줄 원가·소요는 여전히 §5 의 추정이다.
- ⚠️ **그 추정의 전제가 프로덕션과 다르다.** §5 의 ₩20/회차는 flash-lite 단가라야 나오는데,
  프로덕션 content 워커에는 `GEMINI_TRANSLATE_MODEL` 이 **없다**(2026-09-14 `gcloud run jobs
  describe` 실측 — 오버라이드는 `GEMINI_BEAT_ANNOT_MODEL` 하나뿐). 즉 번역은 기본값
  `gemini-2.5-flash` 로 돌고, 출력 단가가 **6.3배**다(₩3,540 vs ₩566 / 1M · `retry.py`).
  **다국어는 언어 수만큼 곱해지는 비용이라** 이 선택이 다른 스테이지보다 크게 작용한다.
- 네이버·틱톡·인스타는 번인만 된다(트랙 API 가 없다). 유튜브 캡션 트랙만 구현.
- 일본어·중국어·태국어는 폰트 번들이 선행이다(위 표).
