...
### 2026-09-14 실측 — 번역 원가·소요, 그리고 **언어 드리프트 버그**

회차 하나(토크 389줄 중 335줄 번역 · 한국어 6,385자)를 같은 입력으로 두 모델에 돌렸다.

| | gemini-2.5-flash | gemini-2.5-flash-lite |
|---|---|---|
| 토큰 | in 8,602 / out 9,694 | in 8,878 / out 7,605 |
| 원가 | **₩37.97** → 925줄 환산 **₩90** | **₩5.57** → 925줄 환산 **₩13** |
| 소요 | 737초 | **16.6초** |

**§5 의 "₩20/회차" 는 flash-lite 기준이었다.** 프로덕션엔 `GEMINI_TRANSLATE_MODEL` 이 없어
기본값 flash 로 도니 실제로는 **회차당 ₩90** 이다(925줄 환산). 언어 3개면 ₩270 이 분석
원가 ₩800 위에 얹힌다(+34%).

⚠️ 소요 737초는 **로컬 쿼터 재시도(429)가 섞인 값**이라 프로덕션 속도로 인용하면 안 된다.
다만 두 모델의 차이(44배)는 재시도만으로 설명되지 않으니, 언어를 늘릴 땐 워커
`DRAIN_MAX_MS`(50분) 안에 끝나는지 확인할 것.

#### 언어 드리프트 — 고쳤다

flash-lite 로 돌린 첫 회차에서 **6배치 중 1배치가 통째로 영어**로 나왔다. 오류도 없고
`refined.id.json` 이 그대로 저장되므로, 유튜브엔 `defaultLanguage=id` 로 영어 자막이 올라간다.

원인을 가르려고 같은 60줄을 반복 호출했다:

| 구간 | 내용 | flash-lite |
|---|---|---|
| 배치 1 | **프랑스어 배우기·파리·프랑스인** | **4번 중 3번 영어** |
| offset 120 | 일반 대화 | 3번 전부 정상 |
| offset 240 | 일반 대화 | 3번 전부 정상 |

**모델 고장이 아니라 그 입력이 목표 언어를 덮은 것**이다 — 본문이 외국어·외국을 다루면
`system_instruction` 의 목표 언어가 긴 입력 뒤쪽에서 밀린다.

고친 방법은 모델 교체가 아니라 **사용자 메시지 맨 뒤에 목표 언어를 한 번 더 박는 것**이다
(캐싱 불변 — 고정부 system 은 그대로). 같은 입력에서 **3/4 → 0/5**, 회차 전체 **6/6 배치 정상**.
`caption-lang.test.ts` 가 그 줄의 존재를 검사한다.

> K콘텐츠 해외 배포는 **본문에 외국 이야기가 자주 나온다**(해외 촬영·외국인 출연·외국어 학습).
> 이 버그가 하필 그 구간에서 터지므로 언어를 늘리기 전에 반드시 들어가야 했다.

### 아직 안 한 것

- ⚠️ **번역 모델을 정해야 한다.** flash ₩90/회차 vs flash-lite ₩13/회차(925줄 환산). 드리프트는
  프롬프트로 막았으니 이제 품질만 보면 된다 — flash-lite 출력도 같은 문장에서 자연스러웠다.
  정하면 `GEMINI_TRANSLATE_MODEL` 을 워커 Job env 에 넣는다.
- ⚠️ **현지화 규칙이 없다.** 프롬프트는 "그 나라 시청자가 쓰는 말로" 라고만 하고 **그 나라가
  뭘 쓰는지는 안 알려준다**(사용자 지적 2026-09-14). 인니어 실측 출력은 `aku/kamu`(반말)까지는
  갔지만 부정형이 `tidak`(문어)이고 어조 입자(`deh/sih/banget`)가 없어 평평하다.
  언어별 말투 규칙을 **한 파일**(`core/common/locale_style.json`)에 두고 자막·메타 프롬프트가
  같이 읽는 구조를 제안한다 — 내용은 원어민 검수가 필요하다(§6 "사람 몫").
- ⚠️ **`VERTEX_LOCATION=global` 이 번역 리전 의도를 덮는다.** `translate_out.py` 는 "자막에
  개인정보가 실릴 수 있어 국내(asia-northeast3)에서 처리" 라고 적어 두었는데, 프로덕션 워커
  env 가 전역으로 `global` 을 건다(beat_annot flash-lite 용으로 넣은 값). 개인정보 처리 위치
  문제라 제품 결정이 필요하다.
- ⚠️ **위 추정의 전제가 프로덕션과 다르다.** §5 의 ₩20/회차는 flash-lite 단가라야 나오는데,
