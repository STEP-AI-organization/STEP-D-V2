...
| 2 | ~~**원장 스키마**~~ | 0023_billing.cjs · billing.ts(순수) + 테스트 23건 | ✅ 2026-08-11 · usage_events 는 append-only 트리거 |
| 3 | 미터링 배선 | ✅ content-pipeline INSERT(멱등) · ⬜ 쿼터 게이트(402) | 원가 가시화는 결제와 독립 |
...
- [x] ~~**PG사 선정**~~ — **나이스페이먼츠**로 확정, 계약 완료 (2026-08-11). 포트원 중계.
- [ ] **포트원에 확인할 것** — ① 사용량 기반 변동금액 청구를 빌링키 정기결제로
      집행하는 것이 허용 범위인지(§4-3) ② 나이스페이먼츠 수수료율
