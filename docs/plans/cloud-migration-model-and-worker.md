...
| 항목 | 단가(추정) | 회차당 10.7분 | 월 12건 |
|---|---:|---:|---:|
| T4 GPU | ~$0.35/h | $0.062 | $0.75 |
| n1-standard-8 | ~$0.38/h | $0.068 | $0.82 |
...
- **쿼터가 안 풀리면?** GEBD 를 로컬 GPU 에 남기고 나머지만 클라우드로 가는 하이브리드가 폴백이다.
  경계는 회차당 10.7분·₩0 이고, `boundaries.json` 을 GCS 에 얹으면 파이프라인이 알아서 소비한다
...
| **인프라 소계** | **~$4 (₩5,500)** |
| **STT Soniox 전환** | **회차당 +₩135 → 월 +₩1,620** |

회차당 API 가 ₩154 → ~₩289 가 된다. **인프라비와 비슷한 수준이라 GPU 의존을 없애는 값으로는 싸다.**

> ⚠️ 이 문서 초안은 Soniox 를 회차당 ₩270 으로 적었다 — 메모리에 있던 **미검증 추정치**를
> 그대로 옮긴 것이었다. 공식 요금은 **$0.10/시간, 화자분리 포함**이다(2026-08-07 확인).
> 58.6분 회차 = $0.098 ≈ ₩135. 실제 청구서로 다시 대조할 것.
