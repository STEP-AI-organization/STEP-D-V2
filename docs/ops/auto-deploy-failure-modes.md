...
활동 시간창 밖에서는 순방이 규칙을 평가하지 않는데, 예전엔 그 구간의 로그가 **0줄**이라
...
| 승인 필요(approve_first) | 보류(hold) → **사람이 확정해야** 다시 잡힌다 | 시간이 지났다고 저절로 풀리면 승인이 무의미하다 |

> ⚠️ **권리 게이트는 2026-08-31 에 제거됐다**(rights_issue 0행 · 실제로 막은 적 없음).
> 보류 사유로 남은 건 `approve_first` 하나뿐이다 — 옛 문서를 보고 되살리지 말 것.
...
| 활동 시간창 기본 | 0~24시(24시간) | `ruleWindow` |
