...
### 결론 — 지금은 **마진 31%** · 화면 자막 읽기를 켜면 본전이 된다

원가가 스위치 하나(`RUN_CHYRON_PER_SEG`)로 두 배 갈린다. **프로덕션은 이 스위치를 끈 채로
돌고 있다** — `stepd-worker-content` 잡 env 에 `RUN_CHYRON_PER_SEG=0` 이 들어 있다
(2026-08-17 잡을 직접 조회해 확인. 리포에는 이 값을 넣는 코드가 없어서 "기본 ON 이겠지" 라고
넘겼다가 틀렸다 — **env 는 짐작하지 말고 조회할 것**).

| | **OFF** (프로덕션 현재) | ON | ON + 배치 모드 |
|---|---|---|---|
| 원가 | **₩1,124** (분당 ₩18.7) | ₩2,385 (분당 ₩39.8) | ₩1,733 (분당 ₩28.9) |
| 판매가 (크레딧 ₩28 × 60분) | ₩1,680 | ₩1,680 | ₩1,680 |
| 카드 수수료(3.2%) 뺀 순매출 | ₩1,626 | ₩1,626 | ₩1,626 |
| **한 편 남는 돈** | **+₩502 (31%)** | -₩759 🔴 | -₩107 (거의 본전) |

**그래서 지금 파는 가격은 문제가 없다.** 대신 **화면 자막 읽기가 꺼진 채로 팔고 있다** —
비용이 아니라 품질 쪽 결정이 매달려 있다. 그게 무엇을 사는 건지는 바로 아래에 적었다.

### 원가 내역

```mermaid
pie showData
    title 60분 한 편 원가 ₩1,124 (프로덕션 현재 · 자막읽기 OFF)
...
| 항목 | 지금 (OFF) | 자막읽기 켜면 | 무엇 |
|---|---|---|---|
| AI 분석 — 장면 이해·구간 선정·서사 | **₩834** | ₩834 | beat 128개 각각 + 쇼츠 QA + 서사·장면분류 (158콜 실측) |
| AI 분석 — **화면 자막 읽기** | — | **+₩1,218** | 자막 세그먼트마다 Vision 1콜 (32.4분에 780콜 실측) · 배치 모드면 ₩609 |
| 받아쓰기 | ₩141 | ₩141 | 음성 → 자막 (Soniox) |
| 서버 연산 | ₩99 | ₩142 | 연산시간 실측 9.2분/32.4분 (자막읽기 켜면 +4분) |
| 영상 렌더 | ₩33 | ₩33 | 쇼츠 20편 + 클립 3편 |
| 음성합성·검색 | ₩17 | ₩17 | 훅 내레이션 + 검색 임베딩 |
| **합계** | **≈ ₩1,124**<br>분당 ₩18.7 | **₩2,385**<br>분당 ₩39.8 | |
...
**지금 구성(자막읽기 OFF)은 편당 ₩502 남으므로 손익분기는 월 113편이다.**

| 월 회차 | 매출 | 지금(OFF) 손익 | 자막읽기 ON 손익 |
|---|---|---|---|
| 30편 | ₩50,400 | -₩41,500 | -₩79,400 |
| 100편 | ₩168,000 | -₩6,400 | -₩132,500 |
| **113편** | ₩189,800 | **≈ ±0** | -₩142,400 |
| 300편 | ₩504,000 | +₩94,000 | -₩284,300 |

- 지금(분당 ₩18.7) → 편당 ₩502 → 손익분기 **월 113편**
- 자막읽기를 켜면(분당 ₩39.8) → 편당 -₩759 → **흑자 구간이 없다**
- 켜고 배치 모드까지(분당 ₩28.9) → 편당 -₩107 → 거의 본전. 가격을 분당 ₩31 로 올리면 흑자
...
| 레버 | 실측 결과 | 상태 |
|---|---|---|
| **자막읽기 스위치 끄기** | -₩1,218 | ✅ **이미 이 상태다**(프로덕션 잡 env). 그래서 지금 마진이 31% 다 — 대신 인물 검색축·화자 실명이 없다 |
| **Gemini 배치 모드** | 자막읽기 **-50%** (₩1,218 → ₩609) | ✅ 구현·적용(2026-08-17) · `GEMINI_BATCH=1`. **자막읽기가 꺼져 있는 동안은 아무 일도 안 한다** — 켤 때를 위한 준비다 |
...
₩107 로 줄고, 판매가를 분당 ₩31 로 올리면 켠 채로 흑자다.
