...
| `clip.subblur` | **원본 자막 블러 검출**(해외 배포 · 2026-09-15). 클립 구간에서 프레임을 샘플(0.5초 간격 · 120초 초과분은 1초)해 `python -m core.vision.subtitle_blur`(PP-OCRv4 det ONNX · onnxruntime)로 burned-in 자막의 사각형+구간을 찾아 `clip.subBlur.events` 에 저장. 렌더(export)가 `editorState.subBlurOn` 일 때 그 자리·그 시간에만 블러를 걸고 번역 자막을 그 위에 굽는다. env `SUBBLUR_DET_MODEL`(det ONNX 경로) 필요 · **content 레인**(GCS 바이트를 당기므로 클라우드) | `POST /api/clips/:id/subblur` (에디터 "원본 자막 블러" 토글) |
| `media.transcode` | 원본을 **프리미어가 읽는 코덱(h264)** 으로 다시 굽는다. 대상은 **vp9·vp8 뿐** — 유튜브에서 받은 원본이 VP9-in-MP4 면 ffmpeg·파이프라인은 멀쩡한데 **프리미어가 영상 트랙을 못 읽어** 편집자 화면엔 오디오 파형만 뜬다(실측 2026-08-31). 오디오는 그대로 복사(`-c:a copy`), 화질은 CRF 20, 결과 길이가 원본과 2초 이상 어긋나면 **덮어쓰지 않는다**. 고객사 업로드 원본(h264·ProRes)은 손대지 않는다 — 재인코딩은 되돌릴 수 없다. **content 레인**(클라우드 · 실측 8~9배속). 사무실 PC 로 보내면 GCS egress 가 붙어 오히려 3배 비싸다(₩45 vs ₩14) — CPU 가 공짜라도 바이트는 공짜가 아니다. **신규 다운로드는 이 잡을 안 탄다** — youtube.download 가 받은 자리에서 바로 변환한다(전송 0원) | `POST /api/media/:id/transcode` · `POST /api/admin/media/transcode-scan`(일괄) |
