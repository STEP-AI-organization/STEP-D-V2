...
>
> | | 이 문서(§4) | 정정 (2026-08-17 실측) |
> |---|---|---|
> | 60분 회차 원가 | ₩994 (분당 17.0) | **≈₩800 (분당 13.3)** — 현행(2026-08-19 flash-lite · 자막읽기 OFF) |
> | 마진 (판매 ₩28/분) | 36% | **약 +51%** · 자막읽기 켜도 flash-lite 면 흑자(~21%) |
> | 손익분기 | 월 88편 | **월 ~69편** (편당 +₩826) |
>
...
업로드 ─┬─ Soniox STT          $0.10/시간          ← 유일한 STT 유료 경로
        ...
        └─ 배포: YouTube API(무료·쿼터) / 네이버(사무실 PC) / Meta·TikTok(스텁)
                    ...
                    ┌──────────────── 매달 나감 (고정비) ────────────────┐
        Cloud SQL db-custom-1-4096 · Cloud Run min=1 · GEBD VM 부팅디스크 · AR 이미지 · GCS · Vercel
                    │
...
> ⚠️ **2026-08-31 재실측 — 이전 표(₩56,600)는 세 군데가 실제와 달랐다.** 사양을 코드·문서가
> 아니라 `gcloud` 로 직접 조회하고, 사용률은 Monitoring API 7일치로 확인했다.
> 틀렸던 것: ① Cloud SQL 이 `db-g1-small` 이 아니라 **`db-custom-1-4096`**(2배)
> ② Cloud Run 이 `min-instances=0` 이 아니라 **`stepd-server` min=1**(월 751 인스턴스-시간 실측)
> ③ Artifact Registry 가 13.8GB 가 아니라 **265.8GB**(이미지 487개 · 정리 정책 부재).

| 항목 | 실측(2026-08-31) | 월 |
|---|---|---|
| Cloud SQL `db-custom-1-4096` + PD_SSD 10GB | CPU **17.5%** · 메모리 58% · 디스크 **0.3/10 GB** | **₩74,000** |
| Cloud Run `stepd-server` **min=1** | 월 **751 인스턴스-시간**(2 vCPU·4GiB 상주) | ~₩22,600 ⚠️ |
| Artifact Registry | 265.8GB → 정리 정책 적용 후 ~40GB 수렴 | ₩37,700 → **₩5,700** |
| GEBD VM 부팅디스크 100GB pd-balanced (정지 중에도 과금) | GEBD 는 실사용 중(8/30 로그) | ₩14,200 |
| GCS `stepd-media` | 76.5GB | ₩2,200 |
| GCS `step-d_cloudbuild` | 41.3GB → 30일 라이프사이클 적용 | ₩1,500 → ~₩300 |
| 워커 Job 폴링 (15분 주기 × 2) | | ₩3,400 |
| Cloud Scheduler (3개까지 무료) | | ₩0 |
| **소계 (정리 전)** | | **≈ ₩155,600** |
| **소계 (정리 후)** | | **≈ ₩122,400** |

⚠️ Cloud Run min-instance 요금은 **활성/유휴 요율이 10배 갈린다.** 위 값은 대부분 유휴라는
가정의 추정이다 — 정확한 값은 청구서 라인아이템으로 확인할 것(`billable_instance_time`
메트릭은 둘을 구분하지 않는다).

**손댈 수 있는 것과 없는 것 (2026-08-31 판단):**

- ✅ **Artifact Registry 정리 정책** — 적용함(최신 20개 보존 + `latest` 태그 보존 + 7일 경과 삭제).
  487개 중 사용 중인 건 3개뿐이었다(`:latest` · 현재 워커 태그 2종, 전부 최신 4위 안). 월 ~₩32,000 절감.
  **배포마다 타임스탬프 태그가 쌓이는데 아무도 안 지워서** 가만히 둬도 계속 커지는 구조였다.
- ✅ **`step-d_cloudbuild` 30일 라이프사이클** — 적용함. 빌드 소스 tarball(회당 ~247MB)이 무한 누적.
- ❌ **Cloud SQL 축소** — CPU 는 17.5% 로 남지만 **메모리가 58% 물려 있어** 티어를 못 내리고,
  Cloud SQL 디스크는 **축소가 아예 불가능**하다(증설만 된다). 위험 대비 효과 없음.
- ⚠️ **Cloud Run min=1 → 0** — 월 ~₩22,600 을 아끼지만 첫 요청에 콜드스타트가 붙는다.
  프록시 경로에서 이미 `ECONNRESET`(freeze/thaw 소켓)을 겪었으므로 안정성 리스크가 절감액보다 크다.
  **권하지 않는다.**
...
| 항목 | 상태 |
|---|---|
| Vercel (`step-d` + `admin` 2 프로젝트) | 🔴 **플랜·요금 미확인.** Hobby 면 ₩0, Pro 면 $20/멤버·월 |
...
> ⚠️ 아래 표의 고정비 ₩56,600 은 **낡았다.** §5 재실측(2026-08-31)으로 정리 후 **₩122,400**
> (정리 전 ₩155,600)이다. Vercel(시트 $20 + 사용량)은 여기 안 잡혀 있다 — 합치면 더 크다.
> 표를 다시 계산하기 전에 회차당 변동비(₩994)도 갱신이 필요하다(CLAUDE.md 는 ₩800 이라 적는다).

| 회차/월 | 변동비 | 고정비(구) | **합계(구)** | 전부 켬 | (참고) 구 infra.md |
|---|---|---|---|---|---|
...
> **손익분기 구조:** 고정비는 회차 수와 무관하다. 그래서 **회차 수를 늘리는 게 단가 인하보다
> 효과가 크다** — 이 결론은 고정비가 커진 지금 더 강해진다.
>
> ⚠️ **2026-08-31 실측: 월 597분(회차 ~10편) 분석에 변동비 ₩7,940 · 고정비 ₩122,400.**
> **GCP 비용의 약 94%가 사용량과 무관하다.** 순수 종량제(₩60/분)로는 구조적으로 회수가
> 안 된다는 뜻이고, 워크스페이스 기본료가 필요하다는 판단의 근거가 이 숫자다.
