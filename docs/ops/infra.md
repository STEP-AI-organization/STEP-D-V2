...
### 2-1. GEBD·YOLO GPU VM — `stepd-gebd-vm`

화면전환 모델(TSN + SJNET)은 CUDA 가 필수라 GPU VM으로 분리했다.
등록 출연자 식별(YOLO26n + ArcFace)도 이 VM의 **별도 `cast` lane**에서 실행한다.
일반 `content.analyze` Cloud Run Job에는 YOLO/InsightFace 의존성을 넣지 않는다.
...
| 항목 | 월 |
|---|---|
| Cloud SQL `db-custom-1-4096` (전용코어 1vCPU·4GB · 2026-09-16 2vCPU 로 올렸다가 **같은 날 원복** — 7일 실측 CPU p95 21% 로 이득 없음 판단) | **₩69,800** ($50.59) |
| Cloud SQL 자동백업 + PITR (WAL·백업 스토리지) | ~₩1,000 (DB 작음 · 청구서 확인) |
| GEBD GPU VM **2대** 부팅디스크 100GB×2 (2026-09-16 2호기 `stepd-gebd-vm-2` — us-central1-a · -b 는 L4 spot 품절이라 존 분산) | ₩27,600 |
| AR 이미지 13.8GB | ₩1,900 |
...
| **소계** | **≈ ₩139,000~154,000** (2026-09-16 GPU 2호기 +₩13,800 · DB 는 원복) |
...
> **2026-09-16 서지 대응**: content Job 은 이제 **enqueue 시점에 서버가 직접 킥**한다
> (`pipeline/worker-kick.ts` → `jobs:run` + `taskCount=4` 오버라이드 · 디바운스 60초). Job 템플릿은
> `tasks=1 · parallelism=4` 로 변경(gcloud 직접 적용 — cloud.sh 는 이 값을 안 건드려 유지된다).
...
고정비(₩139,000~154,000 · 2026-09-16 GPU 2호기 반영) + 폴링(₩3,400) + **분당 ₩15.4**(벤더 11.1 + 인프라 4.3).
...
| 월 분량 | 변동비 | 합계 |
|---|---|---|
| **693분 (2026-09-02 실측 · 30일 40편 · 평균 17.3분)** | ₩11,000 | **≈ ₩153,000~168,000** |
| 1,800분 (60분 × 30편) | ₩27,700 | ≈ ₩170,000~185,000 |
| 6,000분 (60분 × 100편) | ₩92,400 | ≈ ₩235,000~250,000 |
