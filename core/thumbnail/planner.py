...
- captions: 자막 배열 (**기본 1개 · 최대 2개 · 방송사 공식 톤은 절제된 자막**)
  · 각 element: {text, role, position, size}
  · role 은 4종:
    - "main": 메인 카피 · 굵은 고딕 · 하단 좌/중 정렬 지배적 (**반드시 1개 포함**)
    - "subtitle": 메인 바로 위 · 회색 얇은 · 맥락 설명 (**선택 · 있으면 2단**)
    - "quote": 인물 대사 인용 · 흰 손글씨 (**정말 필요할 때만 · 남용 X**)
    - "badge": 핑크 배지 pill (**렉카/자극 톤 · 방송사 공식엔 거의 X · 특별한 경우만**)
  · **원칙**: 기본 [main] 1개 · 맥락 필요하면 [subtitle+main] 2개.
    quote/badge 는 정말 필요한 variant 에만 (예: 리액션 인용 살릴 때). 다 넣으면 렉카 톤.
  · text 는 각각 다른 정보 (중복 X · 부제=맥락 · 메인=사건)
...
**렉카 vs 방송사 톤**:
- 렉카 (X): 배지+인용+부제+메인 다 겹쳐 시끄러움 · 핑크 pill 남발 · 폰트 초대형
- 방송사 (O): 메인 1~2단만 절제 · 색 무드 (다크네이비/크림) · 폰트 적정
