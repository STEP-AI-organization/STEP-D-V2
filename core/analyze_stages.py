...
def run_translate_out(
    *,
    ...
    names = [
        n for n in (
            (m.get("name") or "").strip()
            for m in (cast_registry or []) if isinstance(m, dict)
        ) if n
    ]
    done: dict[str, int] = {}
    for code in codes:
        ts = time.time()
        try:
            cached = load_existing(out_dir, code, len(refined))
            if cached is not None:
                step(f"  해외 자막 {LANGS[code].name_ko}: 기존 결과 재사용 ({len(cached)} 줄)")
                done[code] = 0
            else:
                translated, n = translate_out(refined, code, cast=names)
                save_json(out_dir / f"refined.{code}.json", translated)
                step(f"  해외 자막 {LANGS[code].name_ko}: {n}/{len(refined)} 줄")
                done[code] = n
        except Exception as e:
            step(f"  (해외 자막 {code} 실패 — 한국어는 그대로: {str(e)[:120]})")
        timed(f"translate_out:{code}", ts)
    return done
