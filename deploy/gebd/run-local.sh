...
#   · CHUNK_SEC=300 이 정답 — FEATURE_LEN=300 · 1행/초라 정확히 딱 맞아 0 패딩이 없다
#     (예전 60 은 module.py 가 0.3행/초를 뱉던 시절의 값이다 — 그 조합은 경계가 어긋난다)
#   · CORES=1 이 정답 — parmap 병렬은 산출물을 통째로 날린다(실측 0/12). 1이 27초/청크로 충분히 빠르다
...
rm -rf "$STAGE/scripts" "$STAGE/cla" "$STAGE/prepare"
...
# prepare/ 도 올린다 — module.py 의 video_segmentation 수정(`-c copy` → 재인코딩)이
# 여기 있다. run_long_v3.sh 가 컨테이너 내장본 위에 덮어쓴다. 이게 없으면 feature 가
# 0.3행/초로 나와 경계 시각이 통째로 어긋난다.
