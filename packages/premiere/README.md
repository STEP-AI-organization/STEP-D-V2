...
2. 프로그램을 고른다.
3. 올리는 길은 둘이다:
   - **렌더해서 업로드**(주 동선) — 지금 타임라인에 떠 있는 시퀀스를 H.264 mp4 로 렌더해
     그대로 올린다. 편집이 끝났을 때 원래 누르던 *내보내기* 를 이 버튼이 대신한다.
   - **파일 업로드**(폴백) — 이미 렌더해 둔 파일, 다른 도구로 만든 완성본.
4. 끝나면 배포 가능한 클립으로 등록된다. 게시는 웹에서 한다
...
그래서 **STEP-D 웹을 패널 안에 그대로 띄운다**(`<webview>`). 앱을 새로 만드는 대신 있는 웹을
넣는 것이라 화면을 이식할 필요가 없고, **웹을 배포하면 프리미어 안의 화면도 같이 갱신된다** —
플러그인을 다시 깔지 않아도 된다(플러그인 갱신이 곧 재설치인 이 구조에서 이게 크다).
...
웹이 그걸 쓰게 하는 길이 있다(서버는 `x-stepd-session` 헤더를 이미 받는다).

### 웹에서 "프리미어" 누르면 프리미어가 뜬다

사용자 요구(2026-08-31): *"웹에서 편집 누르면 딱 프리미어 켜지게."*

⚠️ **브라우저가 실행 중인 UXP 패널에 값을 직접 건넬 방법이 없다.** UXP 의 `launchProcess` 는
패널→OS 방향뿐이고, 반대로 OS 가 패널에 인자를 넘기는 문은 없다. 그래서 둘로 나눴다:
...
스킴 등록은 "앱이 꺼져 있을 때 켜주는" 편의다.

**편집자 PC 에 한 번 등록** (관리자 권한 불필요 · HKCU 만 씀):

```powershell
.\launcher\install.ps1              # 등록
.\launcher\install.ps1 -Uninstall   # 제거
```
...
프리미어를 안 써 본 사람을 위한 한 줄 설명 — 같은 일의 세 단계다:

| | 비유 | 편집자가 받는 것 |
|---|---|---|
| **마커** | 책갈피 | 타임라인 눈금에 "여기가 좋다" 표시(이름·점수·추천 id) |
| **서브클립** | 오려둔 조각 | 구간만 떼어낸 가상 클립(파일 복제 없음)이 프로젝트 목록에 |
| **러프컷** | 붙여둔 초벌 | 조각을 **순서대로 늘어놓은 새 타임라인** |

뒤로 갈수록 사람이 할 일이 줄어든다. 러프컷까지 가면 한 바퀴가 닫힌다 — 다듬어서
"렌더해서 업로드" 를 누르면 STEP-D 로 돌아온다.

**원본이 이 PC 에 없어도 된다.** 없으면 STEP-D 에서 받아 프로젝트에 넣고 진행한다
(저장 폴더는 처음 한 번만 묻고 기억 · 같은 크기 파일이 있으면 다시 안 받는다).

⚠️ 서브클립 생성은 **지연 액션**이라 만들어진 항목을 돌려받지 못한다. 그래서 이름 끝에
추천 id 를 박고(`subclipName` — **한 곳에서만** 만든다) 커밋 뒤 그 이름으로 되찾는다.
규칙이 두 곳으로 갈라지는 순간 러프컷이 조용히 빈 시퀀스를 낳는다.

### 추천 구간 탭 (서버 → 프리미어 방향)

서버가 만든 추천을 프리미어 안에서 보고, **줄을 누르면 플레이헤드가 그 구간으로 간다.**
목록은 `GET /api/recommendations?programId=…`(채택 대기·점수 내림차순).

⚠️ **시간 기준이 둘이다.** STEP-D 의 `startTime` 은 **원본 파일 0초 기준**이고 프리미어는
**시퀀스 타임라인 기준**이다. 원본을 통째로 시퀀스에 얹어 편집하는 보통의 경우엔 둘이 같아
그냥 맞는다. 방송 원본처럼 시작 타임코드가 `01:00:00:00` 인 소재를 그대로 얹으면 어긋나므로
서버가 함께 주는 `fps`·`startTimecode` 로 환산해야 한다 — **아직 안 했다.** 대신 프레임
메타가 없는 회차는 목록에 `⚠ 프레임 정합 불가` 라고 적는다. 조용히 틀리는 것보다 낫다.

마커 일괄 생성·서브클립은 다음 단계다.

### 렌더에 관하여

- **프리셋**: `YouTube 1080p HD.epr` (Media Encoder 2026 동봉 · `main.js` `PRESET_PATH`).
  경로가 **Windows 기준으로 하드코딩**돼 있다 — macOS 지원 때 갈라야 한다.
- **AME 큐가 아니라 즉시 렌더**다. 큐에 넘기면 언제 끝났는지 알 수 없어 "딸깍 한 번에
  올라간다" 가 성립하지 않는다. 대신 렌더 동안 프리미어가 바쁘다(내보내기를 누른 것과 같다).
- 결과물은 UXP 임시 폴더에 만들어 올린다. 원본 프로젝트·미디어는 건드리지 않는다.
- 패널은 **시퀀스를 폴링하지 않는다.** 타임라인에서 다른 시퀀스를 열었으면
  "현재 시퀀스" 칸을 눌러 다시 읽는다 — 편집 중에 패널이 계속 프리미어를 건드리지 않으려고.
