...
### 두 갈래 — 웹 편집기 vs 프리미어 (사용자 확정 2026-08-31)

이 제품은 **의도적으로 두 경로**다. 실수로 중복된 게 아니다:

| | 쓰는 사람 | 최종 렌더 | 제목·템플릿 |
|---|---|---|---|
| **웹 편집기** | "알아서 해줘" | STEP-D 서버(ffmpeg + 오버레이) | 서버가 얹는다 |
| **프리미어** | "내 손으로 편집할래" | 프리미어 렌더 | **프리미어가 얹어야 한다** |

프리미어 경로에서는 "렌더해서 업로드" 가 파일을 **그대로** 등록하므로(`directUpload`)
서버가 제목을 얹을 기회가 없다. 그래서 제목은 프리미어 안에서 그려야 한다.

⚠️ **여기서 드리프트가 난다.** 2026-08-31 에 제목 강조색을 `#F3AF4F` 로, 글꼴을 지마켓 산스로
바꿨는데 그건 **서버 쪽만** 바뀐 것이다. 프리미어 경로가 옛 색을 쓰면 같은 프로그램의 쇼츠가
경로에 따라 다르게 나간다 — 고객은 그걸 "일관성이 없다" 고 본다.

**막는 방법: 프리미어에 그리지 말고, 서버가 그린 그림을 받아 얹는다.**
패널은 구간마다 `GET /api/recommendations/:id/title.png?aspect=…` 로 **투명 PNG** 를 받아
V2 트랙에 올린다. 그 PNG 를 그리는 건 **웹 편집기가 쓰는 바로 그 렌더러**(`buildStaticOverlayItems`
+ `renderTextLayerPng`)다 — 글꼴·색·위치가 전부 서버 정본이라 두 경로가 픽셀 단위로 같다.

`aspect` 는 패널이 **열려 있는 타임라인의 실제 프레임 크기**(`sequence.getFrameSize()`)로 정한다.
세로 러프컷이면 1080×1920, 원본 마스터에 마커만 꽂은 가로 타임라인이면 1920×1080 —
비율이 다른 그림을 얹으면 프리미어가 프레임에 맞춰 늘리거나 잘라서 글자 위치가 어긋난다.

⚠️ **처음엔 `.mogrt` 로 갔다가 2026-08-31 에 접었다.** 이유는 사용자 지적 그대로다 —
*"글씨 위치 같은 것도 다 빠질 텐데, 그때그때 생성하면 되지 않을까."* .mogrt 는 위치·크기·글꼴이
파일 안에 박제되고(노출 컨트롤로 뺄 수 있는 건 텍스트·색 정도다), 레이아웃을 바꿀 때마다
사람이 프리미어를 열어 다시 저장해야 한다. 그게 정확히 이 문단이 막으려던 상태였다.
같이 삭제한 것: `GET /api/programs/:id/shorts-style`(색·글꼴 값만 내려주던 라우트) ·
`apps/web/public/mogrt/`.

대가 하나는 분명히 적어 둔다: **PNG 는 프리미어에서 글자를 고칠 수 없다**(이미지다).
문구를 바꾸려면 STEP-D 에서 바꾸고 다시 받는다 — 문구의 정본이 서버라는 계약과 같은 방향이다.
...
| MOGRT · 프리셋 팩 | 코드가 아니라 템플릿 파일 | 무관 (우리 제목은 서버가 PNG 로 그린다 — 위 참조) |
