...
**막는 방법: `.mogrt` 를 껍데기로 두고 값은 서버가 준다.**
템플릿을 통째로 프리미어에 복제하지 말고, 텍스트·색을 **노출된 컨트롤**로 만든 .mogrt 하나만
두고 패널이 서버 값(강조색·글꼴·문구)을 채워 넣는다. 그러면 서버에서 색을 바꿔도 두 경로가
같이 따라온다 — 정본은 계속 한 곳이다.

**.mogrt 를 만들 때 반드시 노출할 것** (필수 그래픽 패널의 "컨트롤로 노출"):
 · 제목 1행 텍스트 · 제목 2행 텍스트
 · 2행 **색상**(강조색) — 이게 노출돼 있어야 서버 값을 밀어 넣을 수 있다
 · (선택) 글꼴 — 노출하면 서버의 글꼴 지정까지 따라온다

노출하지 않으면 값이 .mogrt 안에 **박제**되고, 그때부터 색을 바꿀 때마다 사람이 프리미어를
열어 다시 내보내야 한다. 그게 정확히 이 문서가 막으려는 상태다.
